...
SoftwareSerial mySerial(1, 5); // RX, TX
...
void midinote(){
  if(midibyte[0]>0x8F&&midibyte[0]<0xA0&&midibyte[0]!=0x99){
    if(chan2==1&&chan1==1){
...
if(chan1==0&&a!=midibyte[1]){        
tune_playnote(2,midibyte[1]);
a=midibyte[1];
chan1=1;
}
}
  
    if(midibyte[0]>0x7F&&midibyte[0]<0x90&&midibyte[0]!=0x89){
    if(chan1==1&&a==midibyte[1]){ 
    tune_stopnote(2); 
    a=0;
    chan1=0;
  }
      if(chan2==1&&b==midibyte[1]){ 
    tune_stopnote(1); 
    b=0;
    chan2=0;
  }
      if(chan3==1&&c==midibyte[1]){ 
    tune_stopnote(0); 
    c=0;
    chan3=0;
  }
