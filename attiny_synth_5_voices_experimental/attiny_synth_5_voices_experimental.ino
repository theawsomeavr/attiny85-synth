...
#define NUM_CHANS 5 // number of speaker outputs
bool x[NUM_CHANS];
...
// and do 3-byte instead of 4-byte arithmetic.
  ...
  dospeaker(3);
  dospeaker(4);
 // if(x[0]+x[1]+x[2]+x[3]+x[4]!=state){
      OCR0B=(x[0]+x[1]+x[2]+x[3]+x[4])*10;
 //     state=x[0]+x[1]+x[2]+x[3]+x[4];
  //}
   // Depends on NUM_CHANS==4
...
void startchan5(byte notaa){
 
if(playing[4]==0&&e!=notaa){        
tune_playnote(4,notaa);
e=notaa;
}
}
byte prevnote;
...
if(midichan!=9){
  ...
  if(opcode==0x90&&midibyte[1]!=prevnote){
    ...
    if(playing[0]+playing[1]+playing[2]+playing[3]==4){
    startchan5(midibyte[1]);
    void loop();
    }
